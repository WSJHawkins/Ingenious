...
def scoreFunction(mode, score, maxScore, scoreBoard, otherScoreBoard, turnNo, randomVars):
    ...
    # uses a weighting system based on raising the lowest pieces
        ...
        for s in copyOfScore:
            totalScore = totalScore + multiplayer * s
            multiplayer = multiplayer - 1

        totalScore2 = 0
        multiplayer = 10
        for s in copyOfMaxScore:
            totalScore2 = totalScore2 + multiplayer * s
            multiplayer = multiplayer - 1
    ...
    # Makes decisions after looking at oponents points - optimised weights
        ...
        for i in range(0, 6):
            ...
            totalScore = totalScore + weight3 * (
                    copyOfScore[i] * (otherScoreBoard[i] - scoreBoard[i]) * (1 / (alpha3 * val + beta3)))
            totalScore2 = totalScore2 + weight2 * ((copyOfMaxScore[i] - scoreBoard[i]) * (1 / (alpha2 * val + beta2)))
            totalScore2 = totalScore2 + weight3 * (
                    copyOfMaxScore[i] * (otherScoreBoard[i] - scoreBoard[i]) * (1 / (alpha3 * val + beta3)))
